...
function [TemporalMask] = AssessMotion(Path,soi,param,fid)
    ...
    RP=RP(param.skipped_scans_motionfile+1:end,1:6);

    % Converts the rotational displacement values into [mm] (are given
    % as [rad] by SPM)
    %% Edit for fsl data to use 1:3 instead of 4:6
    RP(:,1:3)=RP(:,1:3)*180/pi;
    ...
    switch param.FD_method
        case 'Power'
            ...
            RPDiffSphere=RPDiff;
            RPDiffSphere(:,1:3)=RPDiffSphere(:,1:3)*50*pi/180; % displacement on surface of sphere with radius 50 mm, arc length = theta(rad) * r
            ...
            if ~isempty(fid)
                WriteInformation(fid,['Average motion:  ',num2str(MeanFD_Power),' [mm]']);
                WriteInformation(fid,['Motion: there are ',num2str(PercentFD_Power),'percent of data scrubbed']);
            end
        otherwise
