...
function [M] = Build_Connectivity_Matrix(IDX,tmp_ss,type,n_items)
    ...
    switch type
        ...
        case 'subjects'
            IDX_full(tmp_ss) = IDX;
%         case 'dims'
%             IDX_full = IDX; % dimensions sampling not implemented
    end
    
    
    % DZ - fastest way to do it (taken from SDHC)
    ...
    % alternatives for computation (not recommended)
    
%     M = zeros(n_items,n_items);
%     for iK=1:length(unique(IDX)) % for every cluster
%         IDX_tmp=zeros(n_items,1);
